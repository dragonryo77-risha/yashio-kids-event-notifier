...
- 前回までに通知済みのURLは `data/state.json` で管理し、重複通知を防止
- 新着があれば LINE Messaging API の「ブロードキャスト配信」で自分に通知
...
GitHub上で新規リポジトリ(Private推奨)を作成し、表示される手順に従って `git remote add origin ...` → `git push -u origin main` を実行してください。
